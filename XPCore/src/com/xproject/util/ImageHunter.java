            ...
            if(!this.validHost(url.getHost())) {
                ...
                if(!this.validContentState(connection.getResponseCode())) {
                    ...
                    if(!this.validFileType(suffix)) {
                        ...
                        String filename = this.savePath.substring(this.savePath.lastIndexOf("/")+1)+suffix;
                        String e = this.getPath(this.savePath, this.filename, suffix);
                        State state = StorageManager.saveFileByInputStream(connection.getInputStream(), e);
                        if(state.isSuccess()) {
                            state.putInfo("url", this.url+"?filename="+filename+"&suffix="+suffix.substring(1));
